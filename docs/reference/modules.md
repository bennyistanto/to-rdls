...
audit.py ← (v1.0 5-layer audit: schema, codelist, semantic, media-type, consumer rules)
...
| `validate_records.py` | v1.0 audit CLI (runs all 5 layers) (wraps src/audit.py); `--enrich` flag runs post-conversion fixes (wraps src/enrich.py) |
...
## v1.0 pipeline and audit modules

Added as the v1.0 (canonical) generation matured; these complement the shared/v0.3 modules above.

| Module | Role |
|--------|------|
| `llm_classify.py` | [v1.0] single Claude call: classify RDLS components + extract all HEVL fields in one pass |
| `extract.py` | [v1.0] HEVL block builders from the LLM response |
| `translate_geonode_v10.py` | [v1.0] GeoNode record -> RDLS v1.0 |
| `translate_stac_v10.py` | [v1.0] STAC Item -> RDLS v1.0 |
| `translate_stac_coclico_v10.py` | [v1.0] CoCliCo Collection -> RDLS v1.0 |
| `codelists.py` | v1.0 codelist utilities (authoritative): `load_codelists_v10()`, `normalise_unit()`, `VALID_*` sets - loaded from the codelist CSVs |
| `audit.py` | the **5-layer** audit validator: `validate()` runs schema, codelist, semantic, media-type, and consumer-rule layers (see [audit-layers](../validation/audit-layers.md)) |
| `enrich.py` | post-conversion enrichment fixes (units, GED4ALL, media types, licenses) |
| `validate_v03.py` | v0.3 semantic validation logic |
| `sources/stac.py` | STAC catalogue adapter (Item / Collection traversal) |

Note: `audit.py`'s own module docstring still reads "three-layer" - stale; `validate()` runs five layers.

...
to-rdls is a modular toolkit for transforming metadata from multiple sources into RDLS v1.0 (or legacy v0.3) JSON records. This document describes what the toolkit can do today.

> **Generation note.** The canonical pipeline is now **v1.0 (LLM-first)** - a single Claude call classifies and extracts a complete record - validated by the [5-layer audit](validation/audit-layers.md). The HEVL-cascade, 4-phase-LLM-review, and 5-pass auto-fix sections below describe the **v0.3 (legacy)** pipeline, still used for some ingestion. See [pipelines/v1.0-llm-first.md](pipelines/v1.0-llm-first.md) for the canonical path.
...
| **GeoNode** (multiple portals) | v1.0 | full pipeline + translator; run across multiple portals |
| **STAC / CoCliCo** | v1.0 | Item-per-record / Collection-per-record translators |
| **DesInventar (UNDRR)** | v0.3/v1.0 | national disaster-loss inventories |
| **NISMOD (ICRA, SDK), MDG, Tomorrow Cities, WBG-UFRA** | v1.0 | program datasets |
| **JKAN legacy catalogue** | v0.3 -> v1.0 | conversion of the original catalogue |

See [pipelines/sources.md](pipelines/sources.md) for the adapter contract and the full source list.
...
```
rdls_{types}-{iso3}_{org}_{titleslug}
```
...
| `iso3` | Lowercase ISO 3166-1 alpha-3, a separate underscore-delimited segment before the org. Omitted for regional/global datasets exceeding `max_countries` (then the id is `rdls_{types}-{org}_{titleslug}`) |
...
See [reference/config.md](reference/config.md) for detailed documentation of each config file.
